...
public class FormulaireTondeuses extends JFrame implements ActionListener {

	private FormulaireTondeusesPanneauAxivIt container = new FormulaireTondeusesPanneauAxivIt();
	...
	private JTextField jtfXTondeuse = new JTextField("abscisse initiale");

	private JTextField jtfYTondeuse = new JTextField("ordonnée initiale");

	private JTextField jtfXCoinDroit = new JTextField("abscisse");

	private JTextField jtfYCoinDroit = new JTextField("ordonnée");

	private URL url = FormulaireTondeuses.class.getResource("/boutonConfirmerNbreTondeuses.png");

	private ImageIcon imageBoutonConfirmer = new ImageIcon(url);

	private JButton boutonConfirmer = new JButton(imageBoutonConfirmer);

	private int nbreDeTondeuses;
	...
	private JLabel positionInitialeDeLaTondeuse = new JLabel("Position Du Véhicule");
	...
	static ArrayList<AutonomousCar> listeTondeuses = new ArrayList<AutonomousCar>(); // Vider
																			// après
																			// l'animation

	static int compteur = 1; // Remettre la valeur initiale après l'animation

	public FormulaireTondeuses(int numeroFormulaire, int nbreDeTondeuses) {

		this.nbreDeTondeuses = nbreDeTondeuses;

		this.numeroFormulaire = numeroFormulaire;

		if (this.nbreDeTondeuses == 1) {
			this.setTitle("4HWC AUTONOMOUS CAR : caractéristiques du véhicule");

		} else // this.nbreDeTondeuses >1
		{
			this.setTitle("4HWC AUTONOMOUS CAR : caractéristiques des " + this.nbreDeTondeuses + " véhicules");
		}
		...
		container.add(formulaire(this.numeroFormulaire, this.nbreDeTondeuses));
	...
	private JPanel formulaire(int numero, int nbreTondeuses) {
		...
		if (nbreTondeuses == 1) {
			...
			suiviFormulaire = new JLabel("Véhicule N° " + numero + " sur " + nbreTondeuses);
		}
		...
		jtfXTondeuse.setPreferredSize(new Dimension(330, 40));

		// jtfInstructions.setFocusable(true);

		jtfXTondeuse.setFont(policeCalibriLight);

		jtfXTondeuse.setForeground(Color.WHITE);

		jtfXTondeuse.setBorder(whiteBorder);

		jtfXTondeuse.setOpaque(false);

		cell4_2.setLayout(new FlowLayout());

		cell4_2.add(jtfXTondeuse);
		...
		// Position Initiale tondeuse

		// JLabel suiviFormulaire = new JLabel("Tondeuse N° i sur 4");

		positionInitialeDeLaTondeuse.setFont(policeAgencyFB);

		positionInitialeDeLaTondeuse.setForeground(Color.WHITE);

		cell5.add(positionInitialeDeLaTondeuse);
		...
		jtfYTondeuse.setPreferredSize(new Dimension(330, 40));

		// jtfInstructions.setFocusable(true);

		jtfYTondeuse.setFont(policeCalibriLight);

		jtfYTondeuse.setForeground(Color.WHITE);

		jtfYTondeuse.setBorder(whiteBorder);

		jtfYTondeuse.setOpaque(false);

		cell6_2.setLayout(new FlowLayout());

		cell6_2.add(jtfYTondeuse);
...

	{
		...
		{
			validationPositions = AutonomousCar.validationPositionsString(jtfXCoinDroit.getText(), jtfYCoinDroit.getText(),
					jtfXTondeuse.getText(), jtfYTondeuse.getText());

			validationOrientation = AutonomousCar
					.validationOrientationTondeuseInitiale(jtfOrientation.getText().toUpperCase());
...

			{
				...
				if (validationOrientation == true) {
					...
					if (validationInstructions == true) {
						...
						FormulaireTondeuses.listeTondeuses
								.add(new AutonomousCar(Integer.parseInt(jtfXCoinDroit.getText().trim()),
										Integer.parseInt(jtfYCoinDroit.getText().trim()),
										Integer.parseInt(jtfXTondeuse.getText().trim()),
										Integer.parseInt(jtfYTondeuse.getText().trim()),
										jtfOrientation.getText().toUpperCase().trim(),
										jtfInstructions.getText().toUpperCase().trim()));

						if (FormulaireTondeuses.compteur == this.nbreDeTondeuses) {
							...
							if (this.nbreDeTondeuses == 1) {
								...
								messageSucces = "Bravo vous avez saisi avec succès les caractéristiques des "
										+ this.nbreDeTondeuses + " véhicules :) !";
							...
							ChargementDeplacementTondeuses.listeTondeuses = FormulaireTondeuses.listeTondeuses;

							ChargementDeplacementTondeuses.origine = "paramètres";

							new ChargementDeplacementTondeuses();

						} else {

							// FormulaireTondeuses.compteur <
							// this.nbreDeTondeuses

							FormulaireTondeuses.compteur++;

							this.setVisible(false);

							new FormulaireTondeuses(FormulaireTondeuses.compteur, this.nbreDeTondeuses);
