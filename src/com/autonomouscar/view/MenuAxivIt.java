...
public class MenuAxivIt extends JFrame implements ActionListener {

	private MenuPanneauAxivIt container = new MenuPanneauAxivIt();
	...
	private JButton boutonIAAxiv = new JButton("Intelligence artificielle 4HWC");
	...
	public MenuAxivIt() {
		...
		boutonIAAxiv.setCursor(cursor);

		boutonIAAxiv.setFont(policeCalibri);

		boutonIAAxiv.setBorder(whiteBorder);

		boutonIAAxiv.setForeground(Color.WHITE);

		boutonIAAxiv.setContentAreaFilled(false);
		boutonIAAxiv.setBorderPainted(true);

		boutonIAAxiv.addActionListener(this);
		...
		cell8_2.add(boutonIAAxiv, BorderLayout.CENTER);
	...
	public void actionPerformed(ActionEvent arg0) {
		if (arg0.getSource() == boutonParDefaut) {
			...
			ChargementDeplacementTondeuses.listeTondeuses = AutonomousCar.recupFichierBase();

			ChargementDeplacementTondeuses.origine = "défaut";
		...
		if (arg0.getSource() == boutonParametres) {
			...
			new ChoixDuNombreDeTondeuses();

		}

		if (arg0.getSource() == boutonIAAxiv) {

			// Changement de fenêtre

			this.setVisible(false);

			ChargementDeplacementTondeuses.listeTondeuses = AutonomousCar.getTondeusesAXIV();

			ChargementDeplacementTondeuses.origine = "IA";
